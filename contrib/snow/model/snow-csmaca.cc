...
NS_LOG_COMPONENT_DEFINE ("snowCsmaCa");

NS_OBJECT_ENSURE_REGISTERED (snowCsmaCa);

TypeId
snowCsmaCa::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::snowCsmaCa")
    .SetParent<Object> ()
    .SetGroupName ("snow")
    .AddConstructor<snowCsmaCa> ()
  ;
  return tid;
}

snowCsmaCa::snowCsmaCa ()
{
...
void
snowCsmaCa::DoDispose ()
{
  m_snowMacStateCallback = MakeNullCallback <void, snowMacState> ();
  m_snowMacTransCostCallback = MakeNullCallback <void, uint32_t> ();
...
void
snowCsmaCa::SetMac (Ptr<snowMac> mac)
{
  m_mac = mac;
}

Ptr<snowMac>
snowCsmaCa::GetMac (void) const
{
  return m_mac;
}

void
snowCsmaCa::SetSlottedCsmaCa (void)
{
...
void
snowCsmaCa::SetUnSlottedCsmaCa (void)
{
  NS_LOG_FUNCTION (this);
  m_isSlotted = false;
}
...
bool
snowCsmaCa::IsSlottedCsmaCa (void) const
{
  NS_LOG_FUNCTION (this);
  return (m_isSlotted);
}
...
bool
snowCsmaCa::IsUnSlottedCsmaCa (void) const
{
  NS_LOG_FUNCTION (this);
  return (!m_isSlotted);
}
...
void
snowCsmaCa::SetMacMinBE (uint8_t macMinBE)
{
...
uint8_t
snowCsmaCa::GetMacMinBE (void) const
{
  NS_LOG_FUNCTION (this);
  return m_macMinBE;
}
...
void
snowCsmaCa::SetMacMaxBE (uint8_t macMaxBE)
{
...
uint8_t
snowCsmaCa::GetMacMaxBE (void) const
{
  NS_LOG_FUNCTION (this);
  return m_macMaxBE;
}
...
void
snowCsmaCa::SetMacMaxCSMABackoffs (uint8_t macMaxCSMABackoffs)
{
...
uint8_t
snowCsmaCa::GetMacMaxCSMABackoffs (void) const
{
  NS_LOG_FUNCTION (this);
...
void
snowCsmaCa::SetUnitBackoffPeriod (uint64_t unitBackoffPeriod)
{
...
uint64_t
snowCsmaCa::GetUnitBackoffPeriod (void) const
{
  NS_LOG_FUNCTION (this);
...
Time
snowCsmaCa::GetTimeToNextSlot (void) const
{
  NS_LOG_FUNCTION (this);
  ...
  if (IsSlottedCsmaCa ())
    {
      ...
      m_randomBackoffEvent = Simulator::Schedule (backoffBoundary, &snowCsmaCa::RandomBackoffDelay, this);

    }
  else
    {
      m_BE = m_macMinBE;
      m_randomBackoffEvent = Simulator::ScheduleNow (&snowCsmaCa::RandomBackoffDelay, this);
    }
}
  ...
  if (IsUnSlottedCsmaCa ())
    {
      ...
      m_requestCcaEvent = Simulator::Schedule (randomBackoff, &snowCsmaCa::RequestCCA, this);
    }
  else
    {
      ...
      if (randomBackoff > timeLeftInCap)
        {
          ...
          m_endCapEvent = Simulator::Schedule (timeLeftInCap, &snowCsmaCa::DeferCsmaTimeout, this);
        }
      else
        {
          m_canProceedEvent = Simulator::Schedule (randomBackoff, &snowCsmaCa::CanProceed, this);
        }
...
snowCsmaCa::CanProceed ()
{
  ...
  if (!m_snowMacTransCostCallback.IsNull ())
    {
      m_snowMacTransCostCallback (transactionSymbols);
    }
  ...
  if (transactionTime > timeLeftInCap)
    ...
    {
      m_requestCcaEvent = Simulator::ScheduleNow (&snowCsmaCa::RequestCCA,this);
    }

}

void
snowCsmaCa::RequestCCA ()
{
  NS_LOG_FUNCTION (this);
...
void
snowCsmaCa::DeferCsmaTimeout ()
{
  NS_LOG_FUNCTION (this);
  m_snowMacStateCallback (MAC_CSMA_DEFERRED);
}

void
snowCsmaCa::PlmeCcaConfirm (snowPhyEnumeration status)
{
              ...
              if (m_CW == 0)
                {
                  ...
                  if (!m_snowMacStateCallback.IsNull ())
                    {
                      NS_LOG_LOGIC ("Notifying MAC of idle channel");
                      m_snowMacStateCallback (CHANNEL_IDLE);
                    }
                }
              else
                {
                  NS_LOG_LOGIC ("Perform CCA again, m_CW = " << m_CW);
                  m_requestCcaEvent = Simulator::ScheduleNow (&snowCsmaCa::RequestCCA, this); // Perform CCA again
                }
            }
          else
            {
      ...
      else
        {
          ...
          if (m_NB > m_macMaxCSMABackoffs)
              ...
              if (!m_snowMacStateCallback.IsNull ())
                {
                  ...
                  m_snowMacStateCallback (CHANNEL_ACCESS_FAILURE);
                }
              return;
            }
          else
            {
              ...
              m_randomBackoffEvent = Simulator::ScheduleNow (&snowCsmaCa::RandomBackoffDelay, this); //Perform another backoff (step 2)
            }
        }
    }
}
...
void
snowCsmaCa::SetsnowMacTransCostCallback (snowMacTransCostCallback c)
{
  NS_LOG_FUNCTION (this);
  m_snowMacTransCostCallback = c;
}


void
snowCsmaCa::SetsnowMacStateCallback (snowMacStateCallback c)
{
  NS_LOG_FUNCTION (this);
  m_snowMacStateCallback = c;
}

void
snowCsmaCa::SetBatteryLifeExtension (bool batteryLifeExtension)
{
...
int64_t
snowCsmaCa::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
...
uint8_t
snowCsmaCa::GetNB (void)
{
  return m_NB;
}

bool
snowCsmaCa::GetBatteryLifeExtension (void)
{

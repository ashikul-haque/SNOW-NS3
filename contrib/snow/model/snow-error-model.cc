...
NS_LOG_COMPONENT_DEFINE ("snowErrorModel");

NS_OBJECT_ENSURE_REGISTERED (snowErrorModel);

TypeId
snowErrorModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::snowErrorModel")
    .SetParent<Object> ()
    .SetGroupName ("snow")
    .AddConstructor<snowErrorModel> ()
  ;
  return tid;
}

snowErrorModel::snowErrorModel (void)
{
...
double
snowErrorModel::GetChunkSuccessRate (double snr, uint32_t nbits) const
{
  double ber = 0.0;

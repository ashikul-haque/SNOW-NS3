 ...
 */
#include "snow-mac.h"
#include "snow-csmaca.h"
#include "snow-mac-header.h"
#include "snow-mac-pl-headers.h"
...
NS_LOG_COMPONENT_DEFINE ("snowMac");

NS_OBJECT_ENSURE_REGISTERED (snowMac);

//IEEE 802.15.4-2011 Table 51
const uint32_t snowMac::aMinMPDUOverhead = 9;
const uint32_t snowMac::aBaseSlotDuration = 60;
const uint32_t snowMac::aNumSuperframeSlots = 16;
const uint32_t snowMac::aBaseSuperframeDuration = aBaseSlotDuration * aNumSuperframeSlots;
const uint32_t snowMac::aMaxLostBeacons = 4;
const uint32_t snowMac::aMaxSIFSFrameSize = 18;

TypeId
snowMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::snowMac")
    .SetParent<Object> ()
    .SetGroupName ("snow")
    .AddConstructor<snowMac> ()
    .AddAttribute ("PanId", "16-bit identifier of the associated PAN",
                   UintegerValue (),
                   MakeUintegerAccessor (&snowMac::m_macPanId),
                   MakeUintegerChecker<uint16_t> ())
    .AddTraceSource ("MacTxEnqueue",
                     "Trace source indicating a packet has been "
                     "enqueued in the transaction queue",
                     MakeTraceSourceAccessor (&snowMac::m_macTxEnqueueTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacTxDequeue",
                     "Trace source indicating a packet has was "
                     "dequeued from the transaction queue",
                     MakeTraceSourceAccessor (&snowMac::m_macTxDequeueTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacTx",
                     "Trace source indicating a packet has "
                     "arrived for transmission by this device",
                     MakeTraceSourceAccessor (&snowMac::m_macTxTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacTxOk",
                     "Trace source indicating a packet has been "
                     "successfully sent",
                     MakeTraceSourceAccessor (&snowMac::m_macTxOkTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacTxDrop",
                     "Trace source indicating a packet has been "
                     "dropped during transmission",
                     MakeTraceSourceAccessor (&snowMac::m_macTxDropTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacPromiscRx",
                     ...
                     MakeTraceSourceAccessor (&snowMac::m_macPromiscRxTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacRx",
                     ...
                     MakeTraceSourceAccessor (&snowMac::m_macRxTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacRxDrop",
                     "Trace source indicating a packet was received, "
                     "but dropped before being forwarded up the stack",
                     MakeTraceSourceAccessor (&snowMac::m_macRxDropTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("Sniffer",
                     "Trace source simulating a non-promiscuous "
                     "packet sniffer attached to the device",
                     MakeTraceSourceAccessor (&snowMac::m_snifferTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("PromiscSniffer",
                     "Trace source simulating a promiscuous "
                     "packet sniffer attached to the device",
                     MakeTraceSourceAccessor (&snowMac::m_promiscSnifferTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("MacStateValue",
                     "The state of snow Mac",
                     MakeTraceSourceAccessor (&snowMac::m_snowMacState),
                     "ns3::TracedValueCallback::snowMacState")
    .AddTraceSource ("MacIncSuperframeStatus",
                     "The period status of the incoming superframe",
                     MakeTraceSourceAccessor (&snowMac::m_incSuperframeStatus),
                     "ns3::TracedValueCallback::SuperframeState")
    .AddTraceSource ("MacOutSuperframeStatus",
                     "The period status of the outgoing superframe",
                     MakeTraceSourceAccessor (&snowMac::m_outSuperframeStatus),
                     "ns3::TracedValueCallback::SuperframeState")
    .AddTraceSource ("MacState",
                     "The state of snow Mac",
                     MakeTraceSourceAccessor (&snowMac::m_macStateLogger),
                     "ns3::snowMac::StateTracedCallback")
    .AddTraceSource ("MacSentPkt",
                     "Trace source reporting some information about "
                     "the sent packet",
                     MakeTraceSourceAccessor (&snowMac::m_sentPktTrace),
                     "ns3::snowMac::SentTracedCallback")
    .AddTraceSource ("IfsEnd",
                     "Trace source reporting the end of an "
                     "Interframe space (IFS) ",
                     MakeTraceSourceAccessor (&snowMac::m_macIfsEndTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}
...
bool
snowMac::GetRxOnWhenIdle ()
{
  return m_macRxOnWhenIdle;
}

void
snowMac::SetRxOnWhenIdle (bool rxOnWhenIdle)
{
  NS_LOG_FUNCTION (this << rxOnWhenIdle);
  m_macRxOnWhenIdle = rxOnWhenIdle;

  if (m_snowMacState == MAC_IDLE)
    {
      if (m_macRxOnWhenIdle)
        {
          m_phy->PlmeSetTRXStateRequest (SNOW_PHY_RX_ON);
...
void
snowMac::SetShortAddress (Mac16Address address)
{
...
void
snowMac::SetExtendedAddress (Mac64Address address)
{
  //NS_LOG_FUNCTION (this << address);
  m_selfExt = address;
}
...
Mac64Address
snowMac::GetExtendedAddress () const
{
  NS_LOG_FUNCTION (this);
  return m_selfExt;
}
void
snowMac::McpsDataRequest (McpsDataRequestParams params, Ptr<Packet> p)
{
  ...
  snowMacHeader macHdr (snowMacHeader::snow_MAC_DATA, m_macDsn.GetValue ());
  m_macDsn++;

  if (p->GetSize () > snowPhy::aMaxPhyPacketSize - aMinMPDUOverhead)
    {
      ...
      if (macHdr.GetDstAddrMode () == SHORT_ADDR)
        {
          ...
          if (shortAddr.IsBroadcast () || shortAddr.IsMulticast ())
            {
              NS_LOG_LOGIC ("snowMac::McpsDataRequest: requested an ACK on broadcast or multicast destination (" << shortAddr << ") - forcefully removing it.");
              macHdr.SetNoAckReq ();
...
void
snowMac::MlmeStartRequest (MlmeStartRequestParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  else
    {
      if (params.m_panCoor)
        {
          ...
          // Setting Channel and Page in the snowPhy
          snowPhyPibAttributes pibAttr;
          ...
          m_phy->PlmeSetAttributeRequest (snowPibAttributeIdentifier::phyCurrentPage,&pibAttr);
        }
      ...
      if (m_macBeaconOrder == 15)
      ...
      else
        {
          ...
          m_beaconEvent = Simulator::ScheduleNow (&snowMac::SendOneBeacon, this);
        }
    }
}
...
void
snowMac::MlmeSyncRequest (MlmeSyncRequestParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  if (params.m_trackBcn)
    {
      ...
      m_trackingEvent = Simulator::Schedule (searchBeaconTime, &snowMac::BeaconSearchTimeout, this);
    }
  else
    {
      m_beaconTrackingOn = false;
...
void
snowMac::MlmePollRequest (MlmePollRequestParams params)
{
  NS_LOG_FUNCTION (this);

  snowMacHeader macHdr (snowMacHeader::snow_MAC_COMMAND, m_macBsn.GetValue ());
  m_macBsn++;
...
void
snowMac::SendOneBeacon ()
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_snowMacState == MAC_IDLE);

  snowMacHeader macHdr (snowMacHeader::snow_MAC_BEACON, m_macBsn.GetValue ());
  m_macBsn++;
  BeaconPayloadHeader macPayload;
  Ptr<Packet> beaconPacket = Create <Packet> ();
  snowMacTrailer macTrailer;

  macHdr.SetDstAddrMode (snowMacHeader::SHORTADDR);
  ...
  if (GetShortAddress () == Mac16Address ("ff:fe"))
    {
      macHdr.SetSrcAddrMode (snowMacHeader::EXTADDR);
      macHdr.SetSrcAddrFields (GetPanId (),GetExtendedAddress ());
    }
  else
    {
      macHdr.SetSrcAddrMode (snowMacHeader::SHORTADDR);
      macHdr.SetSrcAddrFields (GetPanId (), GetShortAddress ());
    }
...
void
snowMac::StartCAP (SuperframeType superframeType)
{
  uint32_t activeSlot;
  ...
  if (superframeType == OUTGOING)
    {
      ...
      m_capEvent =  Simulator::Schedule (endCapTime,
                                         &snowMac::StartCFP, this, SuperframeType::OUTGOING);
  ...
  else
    {
      ...
      m_capEvent =  Simulator::Schedule (endCapTime,
                                         &snowMac::StartCFP, this, SuperframeType::INCOMING);
    }
...
void
snowMac::StartCFP (SuperframeType superframeType)
{
  uint32_t activeSlot;
  ...
  if (superframeType == INCOMING)
    {
      ...
      m_incCfpEvent =  Simulator::Schedule (endCfpTime,
                                            &snowMac::StartInactivePeriod, this, SuperframeType::INCOMING);
    }
  else
    {
      activeSlot = m_superframeDuration / 16;
      ...
      m_cfpEvent =  Simulator::Schedule (endCfpTime,
                                         &snowMac::StartInactivePeriod, this, SuperframeType::OUTGOING);
...
void
snowMac::StartInactivePeriod (SuperframeType superframeType)
{
  ...
  if (superframeType == INCOMING)
    {
      ...
      m_beaconEvent = Simulator::Schedule (endInactiveTime, &snowMac::AwaitBeacon, this);
    }
  else
    {
      ...
      m_beaconEvent =  Simulator::Schedule (endInactiveTime, &snowMac::SendOneBeacon, this);
    }
}

void
snowMac::AwaitBeacon (void)
{
...
void
snowMac::BeaconSearchTimeout (void)
{
  uint64_t symbolRate = (uint64_t) m_phy->GetDataOrSymbolRate (false); //symbols per second
...
snowMac::CheckQueue ()
{
  ...
  if (m_snowMacState == MAC_IDLE && !m_txQueue.empty () && !m_setMacState.IsRunning ())
    {
          ...
          if (!m_ifsEvent.IsRunning ())
            {
              ...
              m_setMacState = Simulator::ScheduleNow (&snowMac::SetsnowMacState, this, MAC_CSMA);
            }
        }
    }
}
...
void
snowMac::SetCsmaCa (Ptr<snowCsmaCa> csmaCa)
{
  m_csmaCa = csmaCa;
}

void
snowMac::SetPhy (Ptr<snowPhy> phy)
{
  m_phy = phy;
}
...
void
snowMac::SetMcpsDataIndicationCallback (McpsDataIndicationCallback c)
{
  m_mcpsDataIndicationCallback = c;
}

void
snowMac::SetMcpsDataConfirmCallback (McpsDataConfirmCallback c)
{
...
void
snowMac::SetMlmeBeaconNotifyIndicationCallback (MlmeBeaconNotifyIndicationCallback c)
{
  m_mlmeBeaconNotifyIndicationCallback = c;
}

void
snowMac::SetMlmeSyncLossIndicationCallback (MlmeSyncLossIndicationCallback c)
{
  m_mlmeSyncLossIndicationCallback = c;
}

void
snowMac::SetMlmePollConfirmCallback (MlmePollConfirmCallback c)
{
  m_mlmePollConfirmCallback = c;
}

void
snowMac::PdDataIndication (uint32_t psduLength, Ptr<Packet> p, uint8_t lqi)
{
  NS_ASSERT (m_snowMacState == MAC_IDLE || m_snowMacState == MAC_ACK_PENDING || m_snowMacState == MAC_CSMA);
  ...
  snowMacTrailer receivedMacTrailer;
  ...
  else
    {
      ...
      if (m_macPromiscuousMode)
      ...
      else
        {
          ...
          acceptFrame = (receivedMacHdr.GetType () != snowMacHeader::snow_MAC_RESERVED);
          ...
          if (acceptFrame
              && (receivedMacHdr.GetType () == snowMacHeader::snow_MAC_BEACON))
            {
          ...
          if (acceptFrame
              && ((receivedMacHdr.GetType () == snowMacHeader::snow_MAC_DATA)
                  || (receivedMacHdr.GetType () == snowMacHeader::snow_MAC_COMMAND))
          ...
          if (acceptFrame)
            {
              ...
              if ((receivedMacHdr.IsData () || receivedMacHdr.IsCommand ()) && receivedMacHdr.IsAckReq ()
                  && !(receivedMacHdr.GetDstAddrMode () == SHORT_ADDR && (receivedMacHdr.GetShortDstAddr ().IsBroadcast () || receivedMacHdr.GetShortDstAddr ().IsMulticast ())))
                {
                  ...
                  if (m_snowMacState == MAC_ACK_PENDING)
                    {
                      m_ackWaitTimeout.Cancel ();
                      PrepareRetransmission ();
                    }
                  else if (m_snowMacState == MAC_CSMA)
                    {
                  ...
                  m_setMacState = Simulator::ScheduleNow (&snowMac::SendAck, this, receivedMacHdr.GetSeqNum ());
                }
              ...
              if (receivedMacHdr.IsBeacon ())
                {
                  ...
                  m_incCapEvent =  Simulator::ScheduleNow (&snowMac::StartCAP, this,SuperframeType::INCOMING);
              ...
              else if (receivedMacHdr.IsAcknowledgment () && m_txPkt && m_snowMacState == MAC_ACK_PENDING)
                {
                  snowMacHeader macHdr;
                  ...
                  if (receivedMacHdr.GetSeqNum () == macHdr.GetSeqNum ())
                    {
                      ...
                      m_setMacState.Cancel ();
                      m_setMacState = Simulator::ScheduleNow (&snowMac::SetsnowMacState, this, MAC_IDLE);
                      // Ack was succesfully received, wait for the Interframe Space (IFS) and then proceed
                      m_ifsEvent = Simulator::Schedule (ifsWaitTime, &snowMac::IfsWaitTimeout, this, ifsWaitTime);
                    }
                  else
                    {
...
snowMac::SendAck (uint8_t seqno)
{
  ...
  snowMacHeader macHdr (snowMacHeader::snow_MAC_ACKNOWLEDGMENT, seqno);
  snowMacTrailer macTrailer;
...
snowMac::AckWaitTimeout (void)
{
  ...
  if (!PrepareRetransmission ())
    {
      SetsnowMacState (MAC_IDLE);
    }
  else
    {
      SetsnowMacState (MAC_CSMA);
    }
}

void
snowMac::IfsWaitTimeout (Time ifsTime)
{
...
bool
snowMac::PrepareRetransmission (void)
{
  NS_LOG_FUNCTION (this);
...
void
snowMac::PdDataConfirm (snowPhyEnumeration status)
{
  NS_ASSERT (m_snowMacState == MAC_SENDING);
          ...
          if (macHdr.IsBeacon ())
            {
              ...
              m_txPkt = 0;
              m_capEvent = Simulator::ScheduleNow (&snowMac::StartCAP, this, SuperframeType::OUTGOING);
          ...
          else if (macHdr.IsAckReq ()) // We have sent a regular data packet, check if we have to wait  for an ACK.
            {
              ...
              m_ackWaitTimeout = Simulator::Schedule (waitTime, &snowMac::AckWaitTimeout, this);
              m_setMacState.Cancel ();
              m_setMacState = Simulator::ScheduleNow (&snowMac::SetsnowMacState, this, MAC_ACK_PENDING);
              return;
            }
          else
            {
...
void
snowMac::PlmeCcaConfirm (snowPhyEnumeration status)
{
...
void
snowMac::PlmeEdConfirm (snowPhyEnumeration status, uint8_t energyLevel)
{
...
void
snowMac::PlmeGetAttributeConfirm (snowPhyEnumeration status,
                                    snowPibAttributeIdentifier id,
...
void
snowMac::PlmeSetTRXStateConfirm (snowPhyEnumeration status)
{
  NS_LOG_FUNCTION (this << status);
...
void
snowMac::PlmeSetAttributeConfirm (snowPhyEnumeration status,
                                    snowPibAttributeIdentifier id)
{
...
void
snowMac::SetsnowMacState (snowMacState macState)
{
  ...
  else if (m_snowMacState == MAC_CSMA && macState == MAC_CSMA_DEFERRED)
    {
      ChangeMacState (MAC_IDLE);
      m_txPkt = 0;
...
snowAssociationStatus
snowMac::GetAssociationStatus (void) const
{
  return m_associationStatus;
}

void
snowMac::SetAssociationStatus (snowAssociationStatus status)
{
  m_associationStatus = status;
}

uint16_t
snowMac::GetPanId (void) const
{
  return m_macPanId;
}

void
snowMac::SetPanId (uint16_t panId)
{
  m_macPanId = panId;
}

void
snowMac::ChangeMacState (snowMacState newState)
{
  NS_LOG_LOGIC (this << " change snow mac state from "
                     << m_snowMacState << " to "
                     << newState);
  m_macStateLogger (m_snowMacState, newState);
  m_snowMacState = newState;
}

uint64_t
snowMac::GetMacAckWaitDuration (void) const
{
...
uint8_t
snowMac::GetMacMaxFrameRetries (void) const
{
  return m_macMaxFrameRetries;
}

void
snowMac::PrintTransmitQueueSize (void)
{
  NS_LOG_DEBUG ("Transmit Queue Size: " << m_txQueue.size ());
}

void
snowMac::SetMacMaxFrameRetries (uint8_t retries)
{
  m_macMaxFrameRetries = retries;
}

bool
snowMac::isCoordDest (void)
{
  NS_ASSERT (m_txPkt);
  snowMacHeader macHdr;
  m_txPkt->PeekHeader (macHdr);
...
void
snowMac::SetAssociatedCoor (Mac16Address mac)
{
  m_macCoordShortAddress = mac;
}

void
snowMac::SetAssociatedCoor (Mac64Address mac)
{
...
uint64_t
snowMac::GetTxPacketSymbols (void)
{
  NS_ASSERT (m_txPkt);

...
#include <ns3/snow-phy.h>
#include <ns3/snow-fields.h>
...
/**
 * \ingroup snow
 * TracedValue callback signature for snowMacState.
 *
 * \param [in] oldValue original value of the traced variable
 * \param [in] newValue new value of the traced variable
 */
typedef void (*snowMacState)(snowMacState oldValue,
                               snowMacState newValue);
...
{
  SNOW_SUCCESS                = 0,
  SNOW_TRANSACTION_OVERFLOW   = 1,
  SNOW_TRANSACTION_EXPIRED    = 2,
  SNOW_CHANNEL_ACCESS_FAILURE = 3,
  SNOW_INVALID_ADDRESS        = 4,
  SNOW_INVALID_GTS            = 5,
  SNOW_NO_ACK                 = 6,
  SNOW_COUNTER_ERROR          = 7,
  SNOW_FRAME_TOO_LONG         = 8,
  SNOW_UNAVAILABLE_KEY        = 9,
  SNOW_UNSUPPORTED_SECURITY   = 10,
  SNOW_INVALID_PARAMETER      = 11
} snowMcpsDataConfirmStatus;
...
struct McpsDataRequestParams
{
  McpsDataRequestParams ()
    : m_srcAddrMode (SHORT_ADDR),
      ...
      m_txOptions (0)
  {}
  snowAddressMode m_srcAddrMode; //!< Source address mode
  snowAddressMode m_dstAddrMode; //!< Destination address mode
...
{
  uint8_t m_msduHandle; //!< MSDU handle
  snowMcpsDataConfirmStatus m_status; //!< The status of the last MSDU transmission
};
/**
 * \ingroup snow
 *
...
struct MlmePollRequestParams
{
  MlmePollRequestParams ()
    : m_coorAddrMode (SHORT_ADDR),
      ...
      m_coorExtAddr ()
  {}
  snowAddressMode m_coorAddrMode; //!< The addressing mode of the coordinator to which the pool is intended.
...
{
  snowMlmeStartConfirmStatus m_status; //!< The status of a MLME-start.request
};
/**
 * \ingroup snow
 *
...
{
  snowSyncLossReason m_lossReason;   //!< The reason for the lost of synchronization.
...
{
  snowMlmePollConfirmStatus m_status; //!< The confirmation status resulting from a MLME-poll.request.
};
/**
 * \ingroup snow
 *
 ...
 * \ingroup snow
 *
 * Class that implements the snow MAC state machine
 */
class snowMac : public Object
{
public:
  /**
   * Get the type ID.
   ...
   */
  void SetCsmaCa (Ptr<snowCsmaCa> csmaCa);
  /**
   * Set the underlying PHY for the MAC.
   *
   * \param phy the PHY
   */
  void SetPhy (Ptr<snowPhy> phy);
  /**
   * Get the underlying PHY of the MAC.
   *
   * \return the PHY
   */
  Ptr<snowPhy> GetPhy (void);
  /**
   ...
   */
  void PdDataConfirm (snowPhyEnumeration status);
  /**
   ...
   */
  void PlmeEdConfirm (snowPhyEnumeration status, uint8_t energyLevel);
  /**
   ...
   */
  void PlmeGetAttributeConfirm (snowPhyEnumeration status,
   ...
   */
  void PlmeSetTRXStateConfirm (snowPhyEnumeration status);
  /**
   ...
   *  @param id the attributed identifier
   */
  void PlmeSetAttributeConfirm (snowPhyEnumeration status,
                                snowPibAttributeIdentifier id);
  /**
   ...
   */
  void SetsnowMacState (snowMacState macState);
  /**
   * Get the current association status.
   *
   * \return current association status
   */
  snowAssociationStatus GetAssociationStatus (void) const;
   ...
   */
  void SetAssociationStatus (snowAssociationStatus status);
  ...
  /**
   * TracedCallback signature for snowMacState change events.
   *
   * \param [in] oldValue The original state value.
   * \param [in] newValue The new state value.
   * \deprecated The snowMacState is now accessible as the
   * TracedValue \c MacStateValue. The \c MacState TracedCallback will
   * be removed in a future release.
   */
  typedef void (*StateTracedCallback)(snowMacState oldState, snowMacState newState);
...
private:
   ...
   */
  void ChangeMacState (snowMacState newState);
  /**
  ...
  /**
   * A trace source that fires when the snowMac changes states.
   ...
   * TracedValue.
   */
  TracedCallback<snowMacState, snowMacState> m_macStateLogger;
  /**
   ...
   */
  TracedValue<snowMacState> m_snowMacState;
  /**
   ...
   */
  snowAssociationStatus m_associationStatus;
  /**

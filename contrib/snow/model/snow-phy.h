...
class Packet;
class SpectrumValue;
class snowErrorModel;
struct snowSpectrumSignalParameters;
class MobilityModel;
...
{
  SNOW_PHY_BUSY  = 0x00,
  SNOW_PHY_BUSY_RX = 0x01,
  SNOW_PHY_BUSY_TX = 0x02,
  SNOW_PHY_FORCE_TRX_OFF = 0x03,
  SNOW_PHY_IDLE = 0x04,
  SNOW_PHY_INVALID_PARAMETER = 0x05,
  SNOW_PHY_RX_ON = 0x06,
  SNOW_PHY_SUCCESS = 0x07,
  SNOW_PHY_TRX_OFF = 0x08,
  SNOW_PHY_TX_ON = 0x09,
  SNOW_PHY_UNSUPPORTED_ATTRIBUTE = 0xa,
  SNOW_PHY_READ_ONLY = 0xb,
  SNOW_PHY_UNSPECIFIED = 0xc // all cases not covered by ieee802.15.4
} snowPhyEnumeration;
...
/**
 * \ingroup snow
 * TracedValue callback signature for snowPhyEnumeration.
 *
 * \param [in] oldValue original value of the traced variable
 * \param [in] newValue new value of the traced variable
 */
  typedef void (* snowPhyEnumeration)(snowPhyEnumeration oldValue,
                                        snowPhyEnumeration newValue);
}  // namespace TracedValueCallback
 ...
 */
typedef Callback< void, snowPhyEnumeration > PlmeCcaConfirmCallback;
 ...
 */
typedef Callback< void, snowPhyEnumeration,uint8_t > PlmeEdConfirmCallback;
 ...
 */
typedef Callback< void, snowPhyEnumeration,
                  snowPibAttributeIdentifier,
                  snowPhyPibAttributes* > PlmeGetAttributeConfirmCallback;
 ...
 */
typedef Callback< void, snowPhyEnumeration > PlmeSetTRXStateConfirmCallback;
 ...
 */
typedef Callback< void, snowPhyEnumeration,
                  snowPibAttributeIdentifier > PlmeSetAttributeConfirmCallback;

/**
 * \ingroup snow
 *
 * Make snowPhy a SpectrumPhy so we can enable the eventual modeling of
 * device interference
 */
class snowPhy : public SpectrumPhy
{
...
public:
   ...
   */
  void PlmeSetTRXStateRequest (snowPhyEnumeration state);
   ...
   */
  void PlmeSetAttributeRequest (snowPibAttributeIdentifier id, snowPhyPibAttributes* attribute);
   ...
   *
   * @param e pointer to snowErrorModel to use
   */
  void SetErrorModel (Ptr<snowErrorModel> e);

  /**
   * get the error model in use
   *
   * @return pointer to snowErrorModel in use
   */
  Ptr<snowErrorModel> GetErrorModel (void) const;
   ...
   * \deprecated The snowPhyEnumeration state is now accessible as the
   * TracedValue \c TrxStateValue.  The \c TrxState TracedCallback will
   * be removed in a future release.
   */
  typedef void (* StateTracedCallback)
    (Time time, snowPhyEnumeration oldState, snowPhyEnumeration newState);
...
protected:
   ...
   * Bit rate is in kbit/s.  Symbol rate is in ksymbol/s.
   */
  static const snowPhyDataAndSymbolRates dataSymbolRates[1];
  /**
   * The preamble, SFD, and PHR lengths in symbols for the different PHY options.
   * See Table 19 and Table 20 in section 6.3 IEEE 802.15.4-2006
   */
  static const snowPhyPpduHeaderSymbolNumber ppduHeaderSymbolNumbers[1];
...
private:
   ...
   */
  void ChangeTrxState (snowPhyEnumeration newState);
   ...
   */
  snowPhyOption GetMyPhyOption (void);
   ...
   */
  void CancelEd (snowPhyEnumeration state);
   ...
   * be removed in a future release.
   */
  TracedCallback<Time, snowPhyEnumeration, snowPhyEnumeration> m_trxStateLogger;
   ...
   */
  Ptr<snowErrorModel> m_errorModel;

  /**
   * The current PHY PIB attributes.
   */
  snowPhyPibAttributes m_phyPIBAttributes;

  // State variables
  /**
   * The current transceiver state.
   */
  TracedValue<snowPhyEnumeration> m_trxState;

  /**
   * The next pending state to applied after the current action of the PHY is
   * completed.
   */
  snowPhyEnumeration m_trxStatePending;
   ...
   * due to interference.
   */
  std::pair<Ptr<snowSpectrumSignalParameters>, bool>  m_currentRxPacket;

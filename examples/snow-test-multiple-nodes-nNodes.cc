...
double snow_signal = -500;
double interference = -500;
...
{
  return 3.92009e-10;
  //return (dbm2w(snow_signal) - dbm2w(noisedbm))/dbm2w(txPower);
}
...
{
  return 1.00172e-11;
  //return (dbm2w(interference) - dbm2w(noisedbm))/dbm2w(jammingTx);
}
/*
double
determineJamming ()
{
  //double comparator = (cost*pow(dbm2w(noisedbm),2))/(calculate_alpha()*calculate_beta());
  double t1 = sqrt ((calculate_alpha () * calculate_beta () * dbm2w (txPower)) / cost_jammer) -
              dbm2w (noisedbm);
  double newJammingTx = t1 / calculate_beta ();
  return w2dbm (newJammingTx);
}

double
determineTX (double jammingTx)
{
  double comparator = calculate_alpha () / (2 * dbm2w (noisedbm));
  double newTx = -20;
  double beta = cost_jammer * pow ((dbm2w (interference) + dbm2w (noisedbm)), 2);
  beta = beta / (calculate_alpha () * dbm2w (txPower));

  if (cost <= comparator)
    {
      NS_LOG_INFO ("case 1");
      double t1 = (calculate_alpha () * cost_jammer);
      double t2 = 4 * beta * pow (cost, 2);
      newTx = t1 / t2;
    }
  else if (cost > comparator && cost <= 2 * comparator)
    {
      NS_LOG_INFO ("case 2");

      double t1 = cost_jammer * pow (dbm2w (noisedbm), 2);
      double t2 = calculate_alpha () * beta;
      newTx = t1 / t2;
    }
  return w2dbm (newTx);
}
...
double
calculateSNR (double nodeTx, double jammerTx)
{
  return ((calculate_alpha () * dbm2w (nodeTx)) /
          (dbm2w (noisedbm) + calculate_beta () * dbm2w (jammerTx)));
}*/
  ...
  else
    {
      ...
      if ((snow_signal + gap) > interference)
      ...
      else
        {
          ...
          NS_LOG_DEBUG ("tx power: " << txPower << " jamming power: " << jammer);
          NS_LOG_DEBUG ("snow_signal: " << snow_signal << " interference: " << interference);
          NS_LOG_DEBUG ("calculated tx: " << energy);
          //NS_LOG_DEBUG("formula tx: "<<determineTX(jammer));

          /*double formula_jammer = determineJamming();
          double formula_tx = determineTX(formula_jammer);
          NS_LOG_DEBUG("formula new jamming againt existing tx: "<<formula_jammer<< "formula new tx: "<<formula_tx);
          NS_LOG_DEBUG("SNR: "<<calculateSNR(formula_jammer, formula_tx));*/
          return energy;
        }
    }
  //return w2dbm(energy);
  ...
  while (1)
    {
      ...
      double secC = (hoppingFreq-470e6)/6e6;
      ...
      if ((searchF == hoppingFreq)&& transmission == 1)
        {
          ...
          tvTransHelper.SetAttribute ("StartFrequency", DoubleValue (hoppingFreq));
      ...
      if (searchF == hoppingFreq && transmission == 1)
        {
          NS_LOG_INFO ("Jammer has detected BS and jamming");
          tvTransHelper.SetAttribute ("StartFrequency", DoubleValue (searchF));
          auto end = std::chrono::high_resolution_clock::now ();
          auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin);
          NS_LOG_DEBUG ("Time to find BS: " << elapsed.count () * 1e-9);
          return;
...
main (int argc, char *argv[])
{
  ...
  char fileName2[100];
  char fileName3[100];
  /*sprintf (fileName1, "./plots/SNOW-game/bw-%dvs-prr.plt", nodeBW);
  sprintf (fileName2, "./plots/SNOW-game/bw-%dvs-epp.plt", nodeBW);
  sprintf (fileName3, "./plots/SNOW-game/bw-%dvs-throughput.plt", nodeBW);*/

  sprintf (fileName1, "./plots/SNOW-game/bw-100vs-prr.plt");
  sprintf (fileName2, "./plots/SNOW-game/bw-100vs-epp.plt");
  sprintf (fileName3, "./plots/SNOW-game/bw-100vs-throughput.plt");
  ...
  //setFreqs(centerF);
  //setTxPowers(txPower);
  //tvTransHelper.SetAttribute ("StartFrequency", DoubleValue (centerF));
  //tvTransHelper.SetAttribute ("BasePsd", DoubleValue (minJammingPower));
          ...
          if (transmission == 1 && intTime % snowTxInterval == 0)
            {
              NS_LOG_INFO ("TX");
              doTransmission (intTime, params, packetSize, snowTxInterval);
              sent += (nSnowNodes*nPackets);
              tvTransHelper.InstallAdjacent (tvTransmitterNodes);
              //jammerEnergymw -= dbm2mw (j);
            }
          Simulator::Run ();
          if (transmission == 1 && sent > 0)
            {
              ...
              NS_LOG_DEBUG ("received: " << g_received << "sent: " << sent);
              total_received += g_received;
              total_retry += g_retry;
              epp += dbm2mw (txPower) * (g_retry + sent);
            }
